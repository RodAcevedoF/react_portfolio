import {
  angrbodaBg,
  angrbodaLogo,
  pinterestBg,
  pinterestLogo,
  hoverHtml,
  hoverCss,
  hoverJs,
  getDoneLogo,
  getDoneBg,
  getDoneBackBg,
  todoApiLogo,
  todoApiBg,
  todoApiBackBg,
  angrbodaBackBg,
  contactBackBg,
  contactBack,
  contactLogo,
  pinterestBackBg,
  thymeLeaf,
  hoverSpring,
  hoverNode,
  hoverBootstrap,
  hoverDocker,
  hoverExpress,
  hoverts,
  chefifyLogo,
  chefifyBack,
  chefifyBackBg,
  reactLogo,
  mui,
  chefifyApiBack,
  chefifyApiBackBg,
  chefifyApiLogo,
  sagepointLogo,
  sagepointBack,
  sagepointBackBg,
  hoverNextjs,
  hoverNestjs,
} from "../assets";
import type { Project } from "../types";

export const projects: Project[] = [
  {
    name: "sagepoint",
    logo: sagepointLogo,
    bgImage: sagepointBack,
    color: "#2BFBB9",
    url: "https://github.com/RodAcevedoF/sagepoint",
    deploy: "https://sagepoint.online",
    backImg: sagepointBackBg,
    description: "projects.sagepoint.description",
    tech: [
      { name: "next", icon: hoverNextjs },
      { name: "mui", icon: mui },
      { name: "ts", icon: hoverts },
      { name: "nest", icon: hoverNestjs },
      { name: "node", icon: hoverNode },
    ],
  },
  {
    name: "chefify",
    logo: chefifyLogo,
    bgImage: chefifyBack,
    color: "#1E90FF",
    url: "https://github.com/RodAcevedoF/project2-chefify2",
    deploy: "https://project2-chefify2.vercel.app",
    backImg: chefifyBackBg,
    description: "projects.chefify.description",
    tech: [
      { name: "react", icon: reactLogo },
      { name: "mui", icon: mui },
      { name: "ts", icon: hoverts },
    ],
  },
  {
    name: "chefify_api",
    logo: chefifyApiLogo,
    bgImage: chefifyApiBack,
    color: "#FF2D95",
    url: "https://github.com/RodAcevedoF/project1-chefify",
    deploy: "https://project2-chefify2.vercel.app",
    backImg: chefifyApiBackBg,
    description: "projects.chefify_api.description",
    tech: [
      { name: "node", icon: hoverNode },
      { name: "css", icon: hoverExpress },
      { name: "ts", icon: hoverts },
    ],
  },
  {
    name: "getdone",
    logo: getDoneLogo,
    bgImage: getDoneBg,
    color: "#38761d",
    url: "https://github.com/RodAcevedoF/project5",
    deploy: "https://getdoneapp.vercel.app/",
    backImg: getDoneBackBg,
    description: "projects.getdone.description",
    tech: [
      { name: "html", icon: hoverHtml },
      { name: "css", icon: hoverCss },
      { name: "js", icon: hoverJs },
    ],
  },
  {
    name: "todo_api",
    logo: todoApiLogo,
    bgImage: todoApiBg,
    color: "#edbc27",
    url: "https://github.com/RodAcevedoF/todo_api",
    deploy: "https://getdoneapp.vercel.app/",
    backImg: todoApiBackBg,
    description: "projects.todo_api.description",
    tech: [
      { name: "node", icon: hoverNode },
      { name: "express", icon: hoverExpress },
      { name: "docker", icon: hoverDocker },
    ],
  },
  {
    name: "angrboda",
    logo: angrbodaLogo,
    bgImage: angrbodaBg,
    color: "#009fb0",
    url: "https://github.com/RodAcevedoF/entrega_practica_DOM",
    deploy: "https://dom-practice-blue.vercel.app/",
    backImg: angrbodaBackBg,
    description: "project.angrboda.description",
    tech: [
      { name: "html", icon: hoverHtml },
      { name: "css", icon: hoverCss },
      { name: "js", icon: hoverJs },
    ],
  },
  {
    name: "contacts_xml",
    logo: contactLogo,
    bgImage: contactBack,
    color: "#5a409d",
    url: "https:github.com/RodAcevedoF/ContactsXML",
    deploy: "https://github.com/RodAcevedoF/ContactsXML",
    backImg: contactBackBg,
    description: "project.contacts_xml.description",
    tech: [
      { name: "spring", icon: hoverSpring },
      { name: "thymeleaf", icon: thymeLeaf },
      { name: "html", icon: hoverHtml },
      { name: "bootstrapp", icon: hoverBootstrap },
    ],
  },
  {
    name: "pinterest",
    logo: pinterestLogo,
    bgImage: pinterestBg,
    color: "#da9393",
    url: "https://github.com/RodAcevedoF/proyecto3",
    deploy: "https://proyecto3-pinterest.vercel.app/",
    backImg: pinterestBackBg,
    description: "project.pinterest.description",
    tech: [
      { name: "html", icon: hoverHtml },
      { name: "css", icon: hoverCss },
      { name: "js", icon: hoverJs },
    ],
  },
];
