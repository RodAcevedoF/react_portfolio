...
import { MailBtn, ContactBtn, MiniLoader } from '../../../components';
import { useModalContext, useScrollContext } from '../../../contexts';
import { type BioSectionProps } from '../../../types';
import { useTranslation, Trans } from 'react-i18next';
const RandomBachWork = lazy(() => import('../cards/RandomBachWork'));
    ...
    <section
      ...
      <div className='flex flex-col w-full text-center'>
        <h4 className='font-[Tungsten] text-[10vw] pl-7 xl:pl-0 md:text-[8vw] lg:text-[6vw] font-[400] self-start tracking-[0.5em]'>
          <Trans
            i18nKey={t('about_sections.about')}
      ...
      <a
        href={pdfPath}
        download
        className='flex relative w-fit cursor-pointer justify-center items-center border rounded-md border-yellow-200/20 p-2 bg-inherit transition-transform duration-300 ease-in-out text-inherit group pl-4 self-center text-center sm:self-start'
        target='_blank'
        rel='noopener noreferrer'>
        <span className=' w-full font-bold text-[1.2em] tracking-[0.5em] font-[Tungsten] transition-opacity duration-200 ease-in-out'>
          {t('about_me.download_cv')}
        </span>
        <span className='bg-blue-400/70 h-[0.2em] w-full left-1.5 translate-x-[30%] opacity-0 absolute transition-all duration-200 ease-in-out rounded-[0.5em] group-hover:opacity-100 group-hover:translate-x-[-2%]' />
      </a>
