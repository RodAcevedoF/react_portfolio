...
export const ExperienceCard = ({ job }: ExperienceListItemProps) => {
  const firstContent = (
    <div className="flex flex-col p-5 bg-black rounded-md min-h-[29rem] w-full">
      ...
      <div className="flex w-full h-fit justify-center items-center gap-4 border-2 border-yellow-200/20 rounded-md px-2 py-1 mb-5">
        <p className="font-[Lilita] text-[1.1em] text-inherit w-fit rounded-sm">{`${job.organization?.title}`}</p>
        <img
  ...
  const secondContent = (
    <div
      className="w-full h-full flex flex-col justify-around items-center p-2"
      style={{ background: `var(--dark-bg)` }}
    >
      <p className="font-[Lilita] text-[1rem] border-2 rounded-md p-2 border-yellow-200/20">
        <span className="text-blue-500">About:</span> {job.description}
      </p>
      <ul className="w-full flex flex-wrap gap-2 items-center justify-center rounded-md py-1">
        ...
        {job.tools?.map((tool) => (
          <li className="flex flex-col justify-center items-center border-2 border-yellow-200/20 hover:translate-y-[-3px] hover:shadow-lg shadow-blue-500 transition-all duration-300 ease-in-out rounded-md p-2 group relative">
            <img
              src={tool.img}
              alt={tool.name}
      ...
      <ul className="w-full flex flex-wrap gap-2 items-center justify-center">
        {job.skills?.map((skill) => (
          <li key={skill}>
            <MiniSkillBtn text={skill} />
          </li>
        ))}
      </ul>
    </div>
