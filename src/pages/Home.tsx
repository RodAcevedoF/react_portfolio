...
import { Title, ToggleParticles, MiniLoader } from '../components';
import { lazy, Suspense } from 'react';
import { useScrollContext } from '../contexts';
import { useTranslation } from 'react-i18next';
import { Trans } from 'react-i18next';
const AnimContainer = lazy(
	() => import('../components/ui/cards/AnimContainer'),
);
const ExpGrid = lazy(() => import('../components/ui/navs/ProjectsGrid'));
		...
		<section className='flex flex-col w-full mt-20 sm:mt-35 xl:mt-38 bg-inherit justify-center items-center'>
			<div className='flex flex-col items-center justify-center w-full gap-8 mb-5 sm:mb-10'>
				<Title
					text={t('home.hero')}
					animateWords
					extraClases='text-[2.4rem] sm:text-[3rem] md:text-[3.5rem] xl:text-[5vw] py-8 px-6'
					width='w-full'
				/>
				<ToggleParticles />
			</div>
